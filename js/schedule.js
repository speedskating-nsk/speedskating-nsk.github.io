export async function loadSchedule() {
	...
	fetch(app).then((res) => res.json().then(
		(data)=> {
			console.log(data)
			// {"result":{"shedule":["9:45-11:14","16:30-17:45 ГЭС","9:45-11:15 ГЭС","15:00-16:15 ГЭС","16:30-17:30 ГЭС","","16:15-17:30 Гэс"],"updated":"2021-12-17T17:32:21.384Z"}}
				const result = data.result;
				console.log(data)
				console.log(result)
				const date = new Date(Date.parse(result.updated));
				var output = "";
				result.schedule.map((value, index)=>output += `<tr><td>${week[index]} </td><td>${value}</td></tr>`);
				const table = `
				<table>
					<colgroup>
						<col width="10%" />
